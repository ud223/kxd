...
namespace KxdLib
{
    public class CourierController :SystemConfigureLib.iController
    {
        ...
        {
            this.SqlText = "INSERT INTO app_courier(courierid, loginname, password, name, phone, code, certificatecode, certificate, companyid, siteid, region, remark, appid, CreateAt, ModifyAt) VALUES('@courierid@', '@loginname@', '@password@', '@name@', '@phone@', '@code@', '@certificatecode@', '', @companyid@, @siteid@, '@region@', '@remark@', '@appid@', '@CreateAt@', '@ModifyAt@'); SELECT * FROM app_courier ORDER BY CreateAt DESC";
        ...
        public void saveValue(Hashtable data)
        {
            ...
            string sql = tools.createSqlText(data, "courierid, CreateAt");
